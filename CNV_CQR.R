...
for(i in num_snps){
  ...
  name = paste(colnames(ukb5)[i], sep ="")
  myform = as.formula(paste("AgeSpexWear ~ ", name, " + UniEdu + Sex + poly(Age,2) + Geno_array + 
  ...
  # True CQR
  quantile_model = tryCatch(rq(formula = myform, data = ukb5, tau = qs, method = "fn"), error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
  ...
  # Linear
  res = try(rma(yi = ses[,1], sei = ses[,2], mods = ~ qs, data = ses[1:length(qs),], control=list(stepadj=0.5)))
  ...
  # MAF
  x             = sum(ukb5[, name], na.rm = TRUE) / (2*sum(!is.na(ukb5[,name])))
  results[i,16] = as.numeric(formatC(min(x, (1-x)), digits = 2, format = "f"))
  
  mod_sumM = summary(lm(formula = myform, data = ukb5))
...
write.csv(results, file = paste("/home/c1669309/CQR/GWAS/results_", ref, ".csv", sep = ""), row.names = F, quote = F)
write.csv(mr, file = paste("/home/c1669309/CQR/GWAS/mr_", ref, ".csv", sep = ""), row.names = F, quote = F)
write.csv(mr2, file = paste("/home/c1669309/CQR/GWAS/mr2_", ref, ".csv", sep = ""), row.names = F, quote = F)
write.csv(df2, file = paste("/home/c1669309/CQR/GWAS/df2_", ref, ".csv", sep = ""), row.names = F, quote = F)
