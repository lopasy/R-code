...
#######################################
{
  ...
  for (snp in 1:num_snps){
    ...
    # Quantile regression
    myform = as.formula(paste("avMSE ~ ", vars[snp_pos[snp]], " + Sex + poly(Age,2) + Array", sep = ""))
...
{hist(training$avMSE, breaks = 100)
hist(testing$avMSE, breaks = 100)

library(tidyverse)
testing = testing %>% mutate(quantile = ntile(avMSE, length(qs)))
...
cor.test(testing$PRS, testing$avMSE)
simple = lm(avMSE~PRS,testing)
#plot(simple)
summary(simple)}
