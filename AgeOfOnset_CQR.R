...
for(i in 1:num_snps){
  ...
  names(final)[51] = name
  myform = as.formula(paste("AgeSpexWear ~ ", name, " + UniEdu + Sex + poly(Age,2) + Array + 
  ...
  results[next_mr,16] = as.numeric(formatC(min(x, (1-x)), digits = 2, format = "f"))
  ...
  results[next_mr,14] = as.numeric(formatC(mod_lev$p.value, digits = 3, format = "e"))
