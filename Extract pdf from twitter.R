...
tweets = read.csv("C:/Users/lopasy/Downloads/twitter/tweets.csv")
...
for(i in links){
  page = html(paste("", tweets[i,10], "", sep = ""))
  ...
  for(j in page){
    ...
    if(isTRUE(class(a)=="try-error")) { missing[paper,] = as.character(tweets[paper,6]) } 
      ...
      pdf_file = file.path("C:/Users/lopasy/Documents/Twitter_papers", "test.pdf")
    name_file = pdf_info(pdf_file)$keys$Title
    name_file = sub(paste0(stopwords, collapse = "|"), "", name_file)
    name_file = gsub("\n", " ", name_file)
    
    if(isTRUE(length(name_file) == 0)) { 
      name_file = pdf_info(pdf_file)$keys$Creator
